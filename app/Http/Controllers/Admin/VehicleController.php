...
class VehicleController extends Controller
{
    ...
    {
        $vehicles = (new Vehicle)->newQuery();

        if (request()->has('search')) {
            $vehicles->where('name', 'Like', '%'.request()->input('search').'%');
        }
        ...
        if (request()->query('sort')) {
            ...
            $vehicles->orderBy($attribute, $sort_order);
        } else {
            $vehicles->latest();
        }

        $vehicles = $vehicles->paginate(5)->onEachSide(2)->appends(request()->query());

        return Inertia::render('Admin/Vehicle/Index', [
            'vehicles' => $vehicles,
            'filters' => request()->all('search'),
            'can' => [
                'create' => Auth::user()->can('vehicles create'),
                'edit' => Auth::user()->can('vehicles edit'),
                'delete' => Auth::user()->can('vehicles delete'),
            ]
        ]);
    }
    ...
    public function accountInfoStore(Request $request)
    {
        $request->validate([
            ...
            'email' => ['required', 'string', 'email', 'max:255', 'unique:vehicles,email,'.\Auth::user()->id],
        ]);
